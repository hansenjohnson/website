...
lin = data.frame(lon, lat)

# make a few points
lon = c(-65.3, -65.7, -64.1)
lat = c(43.4, 43, 42.9)
pts = data.frame(lon, lat)
...
Adding data to the map is relatively straightforward. It's actually nearly the same as adding features to the basemap. The main difference is that you need to actually overwrite and update the variable `map` each time you add something new, hence the `map <- map %>%` with each data type.
