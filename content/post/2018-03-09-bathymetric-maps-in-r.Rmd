...
My biggest issue with this map is that I haven't been able to find an elegent way to add labels to the contour lines in a `ggmap` / `ggplot2` framework. There are ways to do this in `ggplot2` (like with the `direct.label` package), but these fail with projected data used in `ggmap`. Perhaps I'll file an issue with the developers to see if they can shed any light.
